...
import { Star, FileText, MoreVertical, Eye, Trash2, Tag } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export function PDFCard({ doc, viewMode, onView, onToggleFavorite, onDelete }: PDFCardProps) {
    ...
    >
      <div className={`h-36 bg-gradient-to-br ${colors[colorIndex]} flex items-center justify-center relative`}>
        ...
        <Button
          variant="ghost"
          size="icon"
          className="absolute top-2 right-2 h-7 w-7 bg-background/50 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity"
          onClick={(e) => { e.stopPropagation(); onToggleFavorite(doc.id); }}
        >
